...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			if (m->sel->icon) drw_pic(drw, x + mid, (bh - m->sel->ich) / 2, m->sel->icw, m->sel->ich, m->sel->icon);			if (m->sel->isfloating)
...
tile(Monitor *m)
{
	...
	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->wx + mw + panel[2] , m->wy + ty + panel[1], m->ww - mw - (c->bw) - panel[2] - panel[3] + (n > 1 ? gappx : 0), h - (c->bw) - panel[1], 0);
			if (ty + HEIGHT(c) < m->wh)
				ty += HEIGHT(c);
		}
}
